...
fnames = {'lifetime_s', 'trackLengths', 'start', 'catIdx', 'index', 'A', 'A_pstd',...
    'x', 'y', 'sigma_r', 'SE_sigma_r', 'sbA', 'ebA', 'sbSigma_r', 'ebSigma_r', 'gapMat_Ia'};
...
parfor i = 1:nd
    ...
    if ~(exist(fpath, 'file')==2) || ip.Results.Overwrite
        ...
        if nt>0
            ...
            lftData(i).x = NaN(nt,nf,nCh);
            lftData(i).y = NaN(nt,nf,nCh);
            ...
            for k = 1:nt
                ...
                lftData(i).x(k,range,:) = tracks(k).x';
                lftData(i).y(k,range,:) = tracks(k).y';
